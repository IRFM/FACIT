...
def run_facit_ids(machine='west', user='imas_public', shot=58248, run=0, occ=0,optss=0, t1=0., t2=1e4, x1=0.01, x2=0.99, geo=0, Zeff=0, optphinat=0, optasym=0)
	print(' ')
	print('shot           =', shot)
...
#		Bmag = equil.time_slice[kimas].global_quantities.magnetic_axis.b_field_tor
			...
			if dum>=2:
				...
				if len(jacob_imas)==0:
					...
					for ix in range(len(x_imas)):
						jacob_imas[ix,:] =  amin**2*R0*x_imas[ix]
				...
				dpsidx_imas = np.interp(rx_imas,hon_in,dpsidx)
		...
		indx = np.argwhere((rhon_in>x1) & (rhon_in<x2))
